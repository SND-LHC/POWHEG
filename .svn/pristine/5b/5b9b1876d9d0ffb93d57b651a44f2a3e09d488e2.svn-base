...
c     This check was introduced in July 11 2022. It can be removed
c     after some time.
      if(abs(kn_beams(0,1)-kn_beams(0,2)) < 1d-10) then
         tmp=(kn_xb1-kn_xb2)/(kn_xb1+kn_xb2)         
         if(abs(beta-tmp) > 1d-5) then
c safety check for fix to allow for asymmetric beams
            write(*,*) ' symmetric beams, but x1/x2 differs from'//
     1           ' ratio of Born incoming momenta by ',abs(beta-tmp),
     2           ', exiting ...'
            call pwhg_exit(-1)
         endif
      endif
