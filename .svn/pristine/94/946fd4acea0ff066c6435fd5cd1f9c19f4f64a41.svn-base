...
c     beta.  Lorents convention: (t,x,y,z).
     ...
     2           +gamma*beta*pup(4,ipart))
      ...
      implicit none
      include 'LesHouches.h'
      real * 8 sumabs,tmp,total(4)
      integer mu,i,moth,iret
      sumabs=abs(total(1))+abs(total(2))+abs(total(3))+abs(total(3))
      do mu=1,4
      ...
      enddo
      if(sumabs.gt.1d-4) then
         write(*,*) ' lhfm_checkmom'
         write(*,*) ' total mom. cons.:',sumabs
      endif
      do moth=3,nup
         if(istup(moth).eq.2) then
            do mu=1,4
            ...
            if(sumabs.gt.1d-4) then
               write(*,*) ' lhfm_checkmom'
               write(*,*) 'res.',moth,' mom. cons.:',sumabs
            endif
         endif
      enddo
      end
