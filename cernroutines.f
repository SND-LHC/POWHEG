...
C SORTZV Modified to compile on modern gfortran compilers.
C CERN PROGLIB# M101    SORTZV          .VERSION KERNFOR  3.15  820113
C ORIG. 02/10/75
C
      integer A
      dimension A(N1),INDEX(N1)
      dimension F(N1)   ! array of floats for conversion, if needed
      equivalence (ia,fa)
C     
C
      N = N1
...
C
 30   continue
c     The following line causes compilation errors with modern gfortran versions.
c     Convert A to float before calling.
c     CALL SORTTF (A,INDEX,N)
      do i=1,N
c ia and fa are integer and float sharing the same storage         
         ia = A(i)
         F(i) = fa
      enddo
c     The sort routine does not modify the array, only returns the sorted index.
      CALL SORTTF (F,INDEX,N)
C
