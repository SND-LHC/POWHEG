  ...
  for (int iFile = 0; iFile < nFiles; iFile++) {
	...
	for(int i = 0; i <= 3; i++){ // consider each rapidity bin
	  ...
	  for(long unsigned int j = 0; j < vec_weightsID.size(); j++){
	    ...
	    /* APN 19.06.23: 
	     * Commented above line since it saves experimental data into the vec_sim without clearing it.
	     * Below lines fix this bug.
	     */
	    TH1D *pp = (TH1D *)vec_data.at(i).Clone(Form("atlas_sim_rap%d_%s", i, std::to_string(j).c_str()));
            pp->Reset();
            vec_histo_temp.push_back(*pp);
	  }
