...
c     epsilon
c     See also eq. (3.2) of hep-ph/9512328
c     See also eq. (7.32) of hep-ph/9605323
c     See also eq. (2.79) of Florian Koenigs PhD 
c     https://www.uni-muenster.de/imperia/md/content/physik_tp/theses/klasen/koenig_phd.pdf
      if (vflav(3).eq.22) then
         call setborneps(p, vflav, 2, born2, bornjk, bmunu)
         call setborneps(p, vflav, 1, born, bornjk1, bmunu)
         a = 0.d0
         do i = 1, nlegborn
            if (vflav(i).eq.22) cycle ! photon is not colored
            a = a - c(vflav(i))
            do j = 1, nlegborn
               if (vflav(j).eq.22) cycle ! photon is not colored
               if (i.EQ.j) cycle
